...
{
    if (S.find(c) != wstring::npos) // Если такой символ был найден
    {
        return S.find(c); // то вернуть его позицию
    } else return -1;
}

bool eng_word (wstring S) // если символы в строке все находятся в диапазоне английских букв, то true
{
...
{
    int N = S.size();
    if (S[0] < L'А' || S[0] > L'Я' || S.length() < 2) return false; // если первая буква нерусская и незаглавная или если длина меньше двух букв, то false
    for (int i = 1; i < N; i++)
    {
        if (S[i] < L'а' || S[i] > L'я') return false; // проверяем чтобы все буквы были русские
    }
    return true;
}
...
vector<wstring> zapolnenie_vectora ()
{
    vector<wstring> v;
    wstring S;
    wcout << L"Если вы хотите прекратить ввод, введите вместо слова число 1!" << endl;
    while (S != L"1")
    {
        wcout << L"Введите английское слово -> ";
        wcin >> S;
...
vector<wstring> zapolnenie_vectora (int N, int M)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        for (int j = 0; j < M; j++)
        {
            int random = 2 + rand() % 4; // рандомлю число от 2 до 5
            string temp = to_string(random); // привожу к string число random
            wstring w_temp(temp.begin(), temp.end()); // инициализирую wstring значением из string
            S += L" "; // вставляю пробел
            S += w_temp; // вставляю цифру
        }
        v.push_back(S);
    }
    return v;
...
int ex3 (const vector<wstring> &v, int N, int M)
{
    ...
    {
        int count = 0;
        for (wchar_t c : v[i]) // проверяю каждый символ слова, если кол-во найденных пятерок равен M, то
        {
            if (c == '5') count++;
        }
...
void sort_vector (vector<wstring> &v)
{
    ...
    for (int i = 0; i != N-1; i++)
    {
        for (int j = i+1; j != N; j++)
        {
           wchar_t from_string_i = (wchar_t)tolower(v[i][0]); // понижаю первую букву, если вдруг заглавная
           wchar_t from_string_j = (wchar_t)tolower(v[j][0]); // понижаю первую букву, если вдруг заглавная
        ...
        if (menu != 4)
        {
            switch (menu) // переходим непосредственно к выполнению заданий
            {
                case 1:
                {
                    ...
                    if (ex1(S,c) != -1) wcout << ex1(S,c) << L" символа/ов предшествуют первому вхождению символа " << c;
