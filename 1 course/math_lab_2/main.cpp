//ver 3.1 - 08.04.19 // fixed bug с композицией
...
void ex4 (matrix &D, const matrix &A, const matrix &B) // четвертое задание (матрица D 5x5)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        for (int j = 0; j < 5; j++)
        {
            ...
            for (int k = 0; k < 5; k++)
            {
               value += (A[i][k] && B[k][j]); // баг был здесь, было D[i][j] =+ (A[i][k] && B[k][j]), и так как оператора += у вектора нет (компилятор не давал, пришлось поставить =+)
               D[i][j] = value;            // в итоге вместо суммы элементов D[i][j] при k от 0 до 4, элементу D[i][j] в конце цикла просто присваивался A[i][4] * B[4][j], а знак + указывал на знак выражения, а не на сумму
            }
        }
    }
}
...
int main()
{
    ...
    int G(2);
    G =+ 5;
    cout << G; // вместо 7 выведится 5, так как =+, а не +=
    ///TEST
    while (ch == 1)
    {
        generation_vector(M);
