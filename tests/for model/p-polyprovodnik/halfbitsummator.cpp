...
void HalfBitSummator::calculateAndDraw(bool x, bool y)
{
    ...
    int oper1 = !x && y;
    textItems_array[2]->setPlainText({boolToChar(oper1)});
    int oper2 = x && !y;
    textItems_array[3]->setPlainText({boolToChar(oper2)});
    int v = oper1 || oper2;
    textItems_array[5]->setPlainText({boolToChar(v)});
    int u = x && y;
    textItems_array[4]->setPlainText({boolToChar(u)});
}
