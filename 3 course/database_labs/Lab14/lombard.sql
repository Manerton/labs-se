...
--
-- Name: calc_profit(date, date); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.calc_profit(_start date, _end date) RETURNS TABLE("Количество реализованных товаров" bigint, "Общая сумма доходов" double precision)
    LANGUAGE plpgsql
    AS $$
begin
return query
...
ALTER FUNCTION public.calc_profit(_start date, _end date) OWNER TO postgres;
...
SELECT pg_catalog.setval('public."Реализованная сделка_ID_Запись_seq"', 2, true);
