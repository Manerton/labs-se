    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->doubleSpinBox_p->setVisible(false);
}
...
void MainWindow::setupGraph()
{
    ...
    auto min = std::min_element(vector_schemes.begin(),vector_schemes.end(), [](probability_scheme a, probability_scheme b) { return a.getEntropy() < b.getEntropy();});
    customPlot->graph(1)->addData(double(min-vector_schemes.begin())/100,min->getEntropy());
    customPlot->graph(1)->addData(1,0);
...
void MainWindow::on_pushButton_grafik_clicked()
{
    ...
    ui->doubleSpinBox_p->setVisible(true);
    ui->spinBox_n->setEnabled(false);
    uint16_t n = ui->spinBox_n->value();
    // рассчитываем вер. схемы для p от 0 до 1 с шагом 0.01
    for (int i = 0; i <= 100; ++i)
    {
        vector_schemes.push_back(probability_scheme(double(i)/100,n));
    }
    show_probality_scheme();
    setupGraph();
}
