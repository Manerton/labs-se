...
FROM товар_v a,
	(SELECT id_товар, SUM(количество) AS "Количество"
 	...
 	GROUP BY id_товар) b
WHERE b.id_товар = a.id_товар AND b.Количество > 0;
...
(
	...
	id_статус integer NOT NULL DEFAULT 1, -- по умолчанию статус заказа - открыт
	id_пункт_выдачи integer NOT NULL,
	id_менеджер integer,
	время_оформления timestamp NOT NULL,
	время_выдачи timestamp,
	общая_сумма real NOT NULL DEFAULT 0,
...
REVOKE ALL ON FUNCTION add_manager(text, text, text, text, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION update_manager(int, text, text, text, text, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION delete_manager(int) FROM PUBLIC;
...
REVOKE ALL ON FUNCTION add_product_to_warehouse(int, date, int) FROM PUBLIC;
...
SELECT add_product_to_warehouse(3, '2021-02-05', 5);
SELECT add_product_to_warehouse(1, '2021-01-16', 3);
SELECT add_product_to_warehouse(3, '2021-03-10', 3);
...
BEGIN
	...
	ELSE
		...
		VALUES (surname, name, otchestvo, tel, mail) RETURNING id_клиент INTO id_client;
	END IF;
	RETURN id_client;
END;
$BODY$
...
REVOKE ALL ON FUNCTION add_client(text, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION add_client(text, text, text, text, text) TO buyer;

-- Функция, создающая заказ
CREATE OR REPLACE FUNCTION create_order
(IN id_client int, IN id_deliveryPoint int)
RETURNS integer
SECURITY DEFINER
AS $BODY$
DECLARE
	id_order int;
BEGIN
	INSERT INTO заказ (id_клиент, id_пункт_выдачи, время_оформления)
	VALUES (id_client, id_deliveryPoint, now()) RETURNING id_заказ INTO id_order;
	RETURN id_order;
END;
$BODY$
LANGUAGE plpgsql;

REVOKE ALL ON FUNCTION create_order(int, int) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION create_order(int, int) TO buyer;

-- Триггер, который вместо удаления заказа меняет ему статус
...
BEGIN
	...
	WHERE id_заказ = OLD.id_заказ;
	RETURN NULL;
END
$BODY$;

CREATE TRIGGER t_delete_order
    BEFORE DELETE
    ON заказ
    FOR EACH ROW
    EXECUTE PROCEDURE set_cancel_order_status();

-- Триггер, уменьшающий количество товара на складе, 
-- при добавлении товара со склада в заказ
-- и прибавляющий сумму добавленных товаров к общей сумме заказа

CREATE FUNCTION take_product_from_warehouse()
    RETURNS trigger
    LANGUAGE 'plpgsql'
AS $BODY$
DECLARE 
	id_product int;
BEGIN
	UPDATE товар_на_складе 
	SET количество = количество - NEW.количество
	WHERE id_товар_на_складе = NEW.id_товар_на_складе;
	
	id_product := (SELECT id_товар 
					FROM товар_на_складе 
					WHERE id_товар_на_складе = NEW.id_товар_на_складе);
	UPDATE заказ 
	SET общая_сумма = общая_сумма + (SELECT стоимость FROM товар WHERE id_товар = id_product) * NEW.количество
...
CREATE TRIGGER t_insert_order_item
    BEFORE INSERT
    ON позиция_заказа
    FOR EACH ROW
    EXECUTE PROCEDURE take_product_from_warehouse();

-- Функция, добавляющая товар со склада в заказ
CREATE OR REPLACE FUNCTION add_product_to_order
(IN id_order int, IN id_product int, IN required int)
RETURNS void
SECURITY DEFINER
AS $BODY$
DECLARE
	inStock int; 		-- в наличии
	provided int; 		-- предоставлено
	provided_by_step int; 	-- предоставлено на n-ном шагу
	temp_id int;		-- id товара со склада конкретной поставки
	temp_count int;		-- количество товара со склада конкретной поставки
BEGIN
	inStock := (SELECT sum(количество) FROM товар_на_складе WHERE id_товар = id_product);
	provided := 0;
	provided_by_step := 0;
	temp_id := 0;
	temp_count := 0;
	...
	IF (required > inStock)
	THEN
		RAISE 'Product in stock count less than required' USING ERRCODE = 'check_violation';
	ELSE
		WHILE (provided != required)
		LOOP
			temp_id := (SELECT id_товар_на_складе FROM товар_на_складе 
						WHERE (id_товар = id_product AND количество > 0) LIMIT 1);
						
			temp_count := (SELECT количество FROM товар_на_складе 
									WHERE id_товар_на_складе = temp_id);
			
			IF ( (required - provided) >= temp_count )
			THEN 
				provided_by_step := temp_count;
			ELSE
				provided_by_step := required - provided;
			END IF;							
									
			INSERT INTO позиция_заказа (id_заказ, id_товар_на_складе, количество)
				VALUES (id_order, temp_id, provided_by_step);		

			provided := provided + provided_by_step;	
		END LOOP;
	END IF;
END;
$BODY$
