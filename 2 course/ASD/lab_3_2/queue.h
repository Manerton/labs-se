...
private:
    ...
    std::vector<value_type> Lodger_array;
...
public:
    ...
    void replace(const iterator& pos, const_reference b); // замена элемента (pos - какой заменяем, lodger - на какой заменяем)
    ...
    std::vector<value_type> find_before_date(uint16_t Year, uint8_t Month); // поиск по дате (до заданной даты), возвращает контейнер с элементами
    std::vector<value_type> find_after_date(uint16_t Year, uint8_t Month); // поиск по дате (после заданной даты)
    std::vector<value_type> find_by_area(float Area_required); // поиск по требуемой площади (не меньше заданной)
