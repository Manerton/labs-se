...
void Queue::add(const Queue::key_t &key, const Queue::lodger &l)
{
    // проверим ID на допустимость (8 цифр)
    check_length_for_ID(key);

    Lodger_array.push_back({key,l});
    ++count_of_people;
}

// добавление
void Queue::add(const_reference lodger)
{
