...
private:
    ...
    {
        List<data_t> chain;
        key_t& key() {return std::get<0>(*chain.begin());}
        value_t& value() {return std::get<1>(*chain.begin());}
        // инициализация элементом
        node_t(const data_t &tmp)
        {
            chain.push_back(tmp);
        }
        // инициализация ключом без данных
        node_t(const key_t &key)
        {
            chain.push_back({key,value_t()});
        }
        node_t() = default;
    };
// поля
...
// функции
    ...
    auto find_iterator(const key_t &key);
public:
    hash();
    ~hash();
