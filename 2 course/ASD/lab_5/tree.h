...
private:
    ...
    // найти родительский узел, один из дочерних узлов которых имеет указанный key
    node_t *findParentNode(const key_t& key, node_t *node);
    bool check_for_unique_key(const key_t& key);
    size_t find_height(node_t *node) const noexcept;
    void PreOrderCopy(node_t *node);
    node_t* find_min_node(node_t* node);
    node_t* find_max_node(node_t* node);
    void removeNode(node_t *parent, node_t *&target);
public:
    Tree(void) = default;
    virtual ~Tree(void);
    ...
    Tree(Tree&& other);                // -- конструктор переноса --
    Tree& operator=(Tree&& other);     // -- операция перемещания --
