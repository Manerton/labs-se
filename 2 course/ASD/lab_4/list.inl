...
template <typename T>
List<T>::List(List::size_type n, List::const_reference v) : List()
{
    if (n != 0)
    {
...
template <typename T>
List<T>::List(const std::initializer_list<List::value_type> &t) : List()
{
    if (t.size() != 0)
    {
        for (auto& v : t)
...
template <typename T>
List<T>::List(List<T> &&other) noexcept : _size(other.size()), head(other.head), tail(other.tail), end_node(other.end_node)
{
    other._size = 0;
    other.head = nullptr;
    other.tail = nullptr;
...
{
    clear();
    delete end_node;    // удаляем фиктивный конец
}
template <typename T>
...
void List<T>::push_back(List::const_reference N)
{
    Node* newNode = new Node;
    newNode->N = N;
    newNode->next = end_node;   // новый элемент по умолчанию указывает на фиктивный конец
    newNode->prev = nullptr;
    if (empty()) // если очередь пустая
    {
        head = newNode;
    ...
    end_node->prev = tail;  // фиктивный конец указывает на хвост (последний элемент)
    _size++;
}
template <typename T>
...
{
    // идем по итератору до предпоследнего элемента и удаляем одинаковые элементы
    for (Node* tmp = head; tmp != tail; tmp = tmp->next)
    {
        if (tmp->next && (tmp->N == tmp->next->N))
            erase(iterator<T>(tmp));
    }
...
template <typename T> template <typename Y>
typename List<T>::template iterator<Y> List<T>::iterator<Y>::operator++(int)
{
    iterator<Y> it = *this;
    ptr = ptr->next;
    return it;
