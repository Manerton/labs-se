...
Array::iterator Unemployed::find_by_ID(const char *ID)
{
...
const_iterator Unemployed::find_by_ID(const char *ID) const
{
    for (auto it = begin(); !it.isNullptr(); ++it)
    {
        if (strcmp((*it).getID(),ID) == 0)
...
Array::value_type Unemployed::find_elem_by_ID(const char *ID) const
{
    auto it = find_by_ID(ID);
    return *it;
}

Array::iterator Unemployed::find_greater_than_seniority(const UnemployedElem::Seniority &b)
{
...
Array::iterator Unemployed::find_less_than_seniority(const UnemployedElem::Seniority &b)
{
    for (auto it = begin(); it != end(); ++it)
    {
        UnemployedElem::Seniority temp = (*it).getFull_seniority();
