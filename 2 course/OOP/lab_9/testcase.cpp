...
TEST_CASE("Тесты класс List в doctest")
{
    ...
    SUBCASE("Тест создания и присваивания") // -- создание и присваивание --
    {
        ...
        List u{a, b, c};
        CHECK(u.size() == 3);
        auto it = u.begin()->next;  // it указывает на 2 элемент (b)
        u.insert(it,a);
        CHECK(it->N == b);
    }
    ...
    SUBCASE("Тест удаления")              // -- тестирование сложения --
    {
        ...
        UnemployedElem c{"33",25,0,{1,2},{3,7},18543};
        List u{a, b, c};
        auto it = u.begin()->next;  // it указывает на b
        it = u.erase(it);
        CHECK(u.size() == 2);
        CHECK((it->N == c));
    }
...
TEST_CASE("Тест класса Unemployed")
{
    SUBCASE("Тест поиска и удаления по ID")
    {
         ...
         CHECK(list.size() == 3);
         auto it1 = list.begin()->next;
         CHECK(it1->N.toString() == "5231 33 1 5;0 11;4 154355");
         ...
         CHECK(list.size() == 2);
         CHECK(list.begin()->N.toString() == "1314 44 1 4;4 7;3 45500");
         CHECK(list.begin()->next->N.toString() == "33 25 0 1;2 3;7 18543");
         UnemployedElem d("17",19,0,{0,5},{2,4},35444);
         list.change_by_ID("1314",d);
         CHECK(list.size() == 2);
         CHECK(list.begin()->N.toString() == "17 19 0 0;5 2;4 35444");
    }
