...
void Seven::CheckForSevenfold(const Array &arr) const
{
    bool b = all_of(arr.begin(),arr.end(),
                   [] (value_type c) {
                    return (c < num_sys);
                    });
    if (!b) throw not_sevenfold_exception();
}

Seven &Seven::toAdditionalCode(size_type new_N)
{
    size_type diff = new_N - get_size();
    Seven temp(new_N,num_sys-1);
...
Seven &Seven::operator+=(const Seven &b)
{
    Seven max, min;  // -- прибавляю меньшее к большему -- //
...
Seven &Seven::operator-=(const Seven &b)
{
    ...
    Seven add_code_b(b); // -- получаю доп код -- //
...
Seven &Seven::operator*=(const Seven &b)
{
    ...
    for (size_type j = 0; j < N_min; ++j)
    {
        ...
        Seven temp_res(temp.get_size()+j);
...
Seven operator-(const Seven &a, const Seven &b)
{
    Seven t = a;
    t -= b;
    return t;
