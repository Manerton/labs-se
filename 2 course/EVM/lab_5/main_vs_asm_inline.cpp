...
double asm_func(double a, double b)
{
    ...
    __asm { 
        ...
        faddp st(1),st        // st(1) = st(0) + st(1): (a+b), а после st(1) -> st(0) (бывший st(0) выталкивается)
        fyl2x         // 1.0*log2(a+b)
        fldln2        // загружаем константу ln(2)
        fmulp st(1),st        // получаем по формуле ln(a+b) = ln(2) * log2(a+b)
        ...
        fmulp st(1),st       // st(1) = log2(e) * a, а после st(1) -> st(0) (бывший st(0) выталкивается)
        ...
        fld1          // загружаем 1.0
        faddp st(1),st        // прибавляем 1, т.е возвращаем единичку обратно
        ...
        fsin          // синус от b
        fmulp st(1),st        // st(1) = sin(b) * e^a, а после st(1) -> st(0) (бывший st(0) выталкивается)
        fsubp st(1),st        // st(1) = ln(a+b) - e^a*sin(b), т.е получаем числитель в st(0)
        //----------------------------------
        fld a      // скопировали a в st(0) (загрузить a в st(0))
        fld b      // скопировали b в st(0) (загрузить b в st(0))
        fsubp st(1),st        // st(1) = st(1) - st(0): (a-b), а после st(1) -> st(0) (бывший st(0) выталкивается)
        fcos          // считаем cos от (a-b)
        fld a      // скопировали a в st(0) (загрузить a в st(0))
        fsin          // получили sin (a)
        fld b      // скопировали a в st(0) (загрузить a в st(0))
        fsin          // получили sin (b)
        fmulp st(1),st        // st(1) = sin(a) * sin(b), а после st(1) -> st(0) (бывший st(0) выталкивается)
        fsubp st(1),st        // st(1) = cos(a-b) - sin(a)*sin(b), а после st(1) -> st(0) (бывший st(0) выталкивается)
        // получили знаменатель
        //----------------------------------
        fdivp st(1),st       // st(1) = st(1) / st(0): (chis1) / (chis2), а после st(1) -> st(0)
