...
void BitString::CheckForBinary(const string &str) const // проверка содержимого битовой строки, чтобы оно было двоичным
{
    bool res =  all_of(str.begin(),str.end(),
                   [] (char c) {
                    return (c == '0' || c == '1');
                   });
    if (!res) throw EXCEPTION_EXECUTE_HANDLER;
}

void BitString::CallCheckForBinary(const string &str) const
{
   __try
   {
       CheckForBinary(str);
   ...
   {
       MessageBox(NULL, L"Введенная строка содержит лишние символы (она небинарная).", L"ERROR", MB_ICONERROR);
   }
}

void BitString::CheckForLen(const string &str) const // проверка содержимого битовой строки, чтобы оно было не больше 128 бит
{
    if (str.length() > 128) throw EXCEPTION_EXECUTE_HANDLER;
}

void BitString::CallCheckForLen(const string &str) const
{
    __try
    {
        CheckForLen(str);
    ...
    __except(EXCEPTION_EXECUTE_HANDLER)
    {
        MessageBox(NULL, L"Введенная строка слишком длинная (больше 128 символов).", L"ERROR", MB_ICONERROR);
    }
}
