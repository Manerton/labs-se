...
class BitString
{
    ...
    void CheckForBinary(const std::string &str) const; // проверка содержимого битовой строки, чтобы оно было двоичным
    void CallCheckForBinary(const std::string &str) const;
    void CheckForLen(const std::string &str) const; // проверка содержимого битовой строки, чтобы оно было двоичным и не больше 128 бит
    void CallCheckForLen(const std::string &str) const;
...
public:
    ...
    int64_t count_of_SingleBit() const noexcept; // количество битовых единиц в строке
