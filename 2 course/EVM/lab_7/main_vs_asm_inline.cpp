...
void asm_func(double x, double a, int N, double h, double* arr_y)
{
    ...
    __asm {
		mov ebx, arr_y     // это указатель на массив
		mov edi, 0	 // это индекс массива-результата
	 ...
	 cycle:
		...
		// qword ptr - указываем, что сохраняем вещ. число в double, иначе по умолчанию он попытается сохранить как float
		fstp qword ptr [ebx + edi*8] 
		inc edi
