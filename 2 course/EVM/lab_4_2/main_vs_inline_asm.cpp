...
float asm_func(float x, float y)
{
    ...
    __asm {
        ...
        fsubp ST(1),ST                // st(1) = st(1) - st(0): (5x - 10y), а после st(1) -> st(0) (бывший st(0) выталкивается)

        fld y                 // загрузим y в st(0)
        fadd ST, ST(0)        // сложим y+y = 2y
        fdivp ST(1),ST               // st(1) = st(1) / st(0): (5x-10y) / 2y, а после st(1) -> st(0)
        ...
        fsubp ST(1),ST                // st(1) = st(1) - st(0): (6y - 3x), а после st(1) -> st(0), st(2) -> st(1)
        ...
        fdivp ST(1),ST                // st(1) = st(1) / st(0): (6y-3x) / (8y^2), а после st(1) -> st(0), st(2) -> st(1)
                              // в данный момент st(1) лежит результат деления первой дроби, а в st(0) - второй
        fdivp ST(1),ST                // st(1) = st(1) / st(0): (drob1) / (drob2), а после st(1) -> st(0)
